...
void parse_instr_to_struct( uint32_t instr, StruInstr *struInstr )
{

   // opcode, only save low 7-bit, convert to uint8_t, MSB is useless.
   struInstr->opcode = ( uint8_t )( instr & 0x0000007f );
   // rd, shift right 7-bit ( discard opcode ) and only save 5-bit
   // convert to uint8_t, MSB 3-bit is useless.
   struInstr->rd = ( uint8_t )( ( instr >> 7 ) & 0x0000001f );
   // func3, shift right 12-bit ( discard opcode and rd ) and only save 3-bit
   // convert to uint8_t, MSB 5-bit is useless.
   struInstr->func3 = ( uint8_t )( ( instr >> 12 ) & 0x00000007 );
   // rs1, shift right 15-bit ( discard opcode, rd, func3 ) and only save 5-bit
   // convert to uint8_t, MSB 3-bit is useless.
   struInstr->rs1 = ( uint8_t )( ( instr >> 15 ) & 0x0000001f );
   // rs2, shift right 20-bit ( discard opcode, rd, func3, rs1 ) and only save 5-bit
   // convert to uint8_t, MSB 3-bit is useless.
   struInstr->rs2 = ( uint8_t )( ( instr >> 20 ) & 0x0000001f );
   // rs2, shift right 25-bit ( discard opcode, rd, func3, rs1, rs2 ) and only save 5-bit
   // convert to uint8_t, MSB 3-bit is useless.
   struInstr->func7 = ( uint8_t )( instr >> 25 );
   // imm_i, shift right 20-bit ( discard opcode, rd, func3, rs1 ) and only save 20-bit
   // convert to uint32_t, MSB 12-bit is useless.
   struInstr->imm_i = ( uint32_t )( instr >> 20 );
   if ( ( instr >> 31 ) == 1 )
      struInstr->imm_i += 0xffffff000;
   // imm_s, shift right 20-bit ( discard opcode, rd, func3, rs1 ) and only save 20-bit
   // convert to uint32_t, MSB 12-bit is useless.
   struInstr->imm_s = ( uint32_t )( ( ( instr >> 20 ) & 0x00000fe0 ) + ( ( instr >> 7 ) & 0x0000001f ) );
   if ( ( instr >> 31 ) == 1 )
      struInstr->imm_s += 0xfffff000;
   // imm_b, instr 31, 7, 30-25, 11-8 bit with sign bit extention
   // convert to uint32_t, LSB is 0,
   struInstr->imm_b = ( uint32_t )( ( ( instr >> 29 ) & 0x00001000 ) + ( ( instr << 4 ) & 0x00000800 ) + ( ( instr >> 20 ) & 0x000007e0 ) + ( ( instr >> 7 ) & 0x0000001e ) );
   if ( ( instr >> 31 ) == 1 )
      struInstr->imm_b += 0xffffe000; // sign bit extention
   // imm_j, instr 31, 19-12, 20, 30-21 bit with sign bit extention
   // convert to uint32_t, LSB is 0,
   struInstr->imm_j = ( uint32_t )( ( ( instr >> 12 ) & 0x00080000 ) + ( ( instr ) & 0x00007800 ) + ( ( instr >> 9 ) & 0x00000400 ) + ( ( instr >> 20 ) & 0x000007fe ) );
   if ( ( instr >> 31 ) == 1 )
      struInstr->imm_j += 0xffe00000; // sign bit extention
   // imm_u, instr 31-12 bit, with low 12-bit set to 0
   // convert to uint32_t
   struInstr->imm_u = ( uint32_t )( instr & 0xfffff000 );
   // imm_z, instr 19-15 bit, with high 27-bit set to 0
   // convert to uint32_t
   struInstr->imm_z = ( uint32_t )( ( instr >> 15 ) & 0x0000001f );
}
